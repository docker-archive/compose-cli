...
func upCommand(p *projectOptions, contextType string, w WithComposeService) *cobra.Command {
	opts := upOptions{
	...
	upCmd := &cobra.Command{
		...
		RunE: w(func(ctx context.Context, s compose.Service, args []string) error {
			...
			case store.LocalContextType, store.DefaultContextType, store.EcsLocalSimulationContextType:
				...
				return runCreateStart(ctx, s, opts, args)
			default:
				return runUp(ctx, s, opts, args)
			}
		}),
	}
	flags := upCmd.Flags()
...
func runUp(ctx context.Context, s compose.Service, opts upOptions, services []string) error {
	...
	_, err = progress.Run(ctx, func(ctx context.Context) (string, error) {
		return "", s.Up(ctx, project, compose.UpOptions{
			Detach:    opts.Detach,
...
func runCreateStart(ctx context.Context, s compose.Service, opts upOptions, services []string) error {
	project, err := setup(*opts.composeOptions, services)
	if err != nil {
		return err
	}
	...
	_, err = progress.Run(ctx, func(ctx context.Context) (string, error) {
		err := s.Create(ctx, project, compose.CreateOptions{
			Services:             services,
		...
		if opts.Detach {
			err = s.Start(ctx, project, compose.StartOptions{})
		}
		return "", err
	})
	if err != nil {
