...
func runCommand(p *projectOptions, w WithComposeService) *cobra.Command {
	opts := runOptions{
...
func runRun(ctx context.Context, s compose.Service, opts runOptions) error {
	...
	_, err = progress.Run(ctx, func(ctx context.Context) (string, error) {
		return "", startDependencies(ctx, s, *project, opts.Service)
	})
	if err != nil {
		return err
	}
	...
	exitCode, err := s.RunOneOffContainer(ctx, project, runOpts)
	if exitCode != 0 {
		errMsg := ""
		if err != nil {
			errMsg = err.Error()
...
func startDependencies(ctx context.Context, s compose.Service, project types.Project, requestedServiceName string) error {
	...
	if err := s.Create(ctx, &project, compose.CreateOptions{}); err != nil {
		return err
	}
	if err := s.Start(ctx, &project, compose.StartOptions{}); err != nil {
		return err
	}
	return nil
}
