...
func imagesCommand(p *projectOptions, w WithComposeService) *cobra.Command {
	opts := imageOptions{
		projectOptions: p,
	}
	imgCmd := &cobra.Command{
		...
		RunE: w(func(ctx context.Context, s compose.Service, args []string) error {
			return runImages(ctx, s, opts, args)
		}),
	}
...
func runImages(ctx context.Context, s compose.Service, opts imageOptions, services []string) error {
	projectName, err := opts.toProjectName()
	if err != nil {
		return err
	}

	images, err := s.Images(ctx, projectName, compose.ImagesOptions{
		Services: services,
	})
	if err != nil {
		return err
