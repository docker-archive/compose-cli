...
func removeCommand(p *projectOptions, w WithComposeService) *cobra.Command {
	opts := removeOptions{
		projectOptions: p,
	}
	cmd := &cobra.Command{
...
Any data which is not in a volume will be lost.`,
		RunE: w(func(ctx context.Context, s compose.Service, args []string) error {
			return runRemove(ctx, s, opts, args)
		}),
	}
	f := cmd.Flags()
...
func runRemove(ctx context.Context, s compose.Service, opts removeOptions, services []string) error {
	...
	if opts.stop {
		_, err = progress.Run(ctx, func(ctx context.Context) (string, error) {
			err := s.Stop(ctx, project, compose.StopOptions{})
			return "", err
		})
		if err != nil {
			return err
	...
	reosurces, err := s.Remove(ctx, project, compose.RemoveOptions{
		DryRun: true,
	})
	if err != nil {
		return err
	...
	_, err = progress.Run(ctx, func(ctx context.Context) (string, error) {
		_, err = s.Remove(ctx, project, compose.RemoveOptions{
