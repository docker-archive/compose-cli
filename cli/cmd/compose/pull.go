...
func pullCommand(p *projectOptions, w WithComposeService) *cobra.Command {
	opts := pullOptions{
		projectOptions: p,
	}
	cmd := &cobra.Command{
		...
		RunE: w(func(ctx context.Context, s compose.Service, args []string) error {
			...
			return runPull(ctx, s, opts, args)
		}),
	}
	flags := cmd.Flags()
...
func runPull(ctx context.Context, s compose.Service, opts pullOptions, services []string) error {
	...
	if opts.quiet {
		return s.Pull(ctx, project, apiOpts)
	}
