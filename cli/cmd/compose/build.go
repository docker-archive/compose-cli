...
func buildCommand(p *projectOptions, w WithComposeService) *cobra.Command {
	opts := buildOptions{
		projectOptions: p,
	}
	cmd := &cobra.Command{
		...
		RunE: w(func(ctx context.Context, s compose.Service, args []string) error {
			...
			return runBuild(ctx, s, opts, args)
		}),
	}
...
func runBuild(ctx context.Context, s compose.Service, opts buildOptions, services []string) error {
	...
	_, err = progress.Run(ctx, func(ctx context.Context) (string, error) {
		return "", s.Build(ctx, project, compose.BuildOptions{
			Pull:     opts.pull,
